...
INFO="$(curl --silent -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/BurntSushi/ripgrep/releases?per_page=1 | jq "$JQ_FILTER")"

for VERSION in $(jq -r 'keys | join("\n")' <<<$INFO); do
  for PLATFORM in $(jq -r ".[\"$VERSION\"] | keys | join(\"\n\")" <<<$INFO); do
    FILENAME=$(jq -r ".[\"$VERSION\"][\"$PLATFORM\"].filename" <<<$INFO)
    SHA256=$(curl -fLs "https://github.com/BurntSushi/ripgrep/releases/download/$VERSION/$FILENAME" | sha256sum | xxd -r -p | base64)
    INFO=$(jq ".[\"$VERSION\"][\"$PLATFORM\"].sha256 = \"sha256-$SHA256\"" <<<$INFO)
  done
done

echo -n "RIPGREP_VERSIONS = "
echo $INFO | jq -M

echo ""
echo "Copy the version info into lib/private/ripgrep_toolchain.bzl"
